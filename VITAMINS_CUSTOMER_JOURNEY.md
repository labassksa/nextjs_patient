...
<td style="padding:7px 12px">Lab org portal — patient brings existing blood results, no new test ordered</td>
...
<td style="padding:7px 12px">TikTok / Instagram / Snap → landing page → lab test bundled in package</td>
<td style="padding:7px 12px">Self-pay (vitamins subscription)</td>
</tr>
<tr style="background:#fff8e8">
<td style="padding:7px 12px; font-weight:700; color:#7a5500">Secondary</td>
<td style="padding:7px 12px">3</td>
<td style="padding:7px 12px"><strong>Lab Customer (Pre-Test)</strong></td>
<td style="padding:7px 12px; white-space:nowrap">Phase 2 🚧</td>
...
## TYPE 1 🔴 PRIMARY (Phase 1 ✅): Patient Visits Lab → Gets Post-Test Consultation → Doctor Reviews Existing Blood Results → Prescribes Vitamins

> The patient visits the lab independently, gets their blood work done, and arrives at the Labass consultation with those results in hand. The vitamins package does **NOT** include ordering a new lab test — the doctor uses the existing results to prescribe. This is the highest-converting customer type because the need is already proven by data.

<div dir="rtl" style="font-size:0.85em; line-height:2; padding-right:1em">
<ol>
<li>يزور العميل المختبر ويجري تحاليل الدم</li>
...
| 2 | **Lab — org portal** | B2B | Opens the Labass org portal. Enters the patient's phone number. Selects *post-test* consultation type. Uploads the lab results PDF. Submits. | <small>• Remaining bundle count decreases by 1</small> |
...
<li>يدخل العميل الاستشارة، ثم يقوم الدكتور بإرسال رابط صفحة الفيتامينات والاشتراك بناءً على الأعراض والأهداف الصحية فقط — <strong>لا يوجد تحليل دم في هذه المرحلة</strong></li>
<li>يدخل العميل إلى الصفحة، ويقوم بتعبئة المعلومات المطلوبة، ثم إتمام عملية الدفع — <em>(مشترك)</em></li>
...
<li>يدخل العميل إلى الاستشارة الجديدة، ثم يقوم الدكتور بإصدار وصفة طبية بناءً على الأعراض والأهداف الصحية — <em>(مشترك)</em></li>
<li>تظهر الباقة التي اشترك فيها العميل للطبيب داخل تطبيق الدكتور عند قبول الاستشارة — <em>(مشترك)</em></li>
<li><strong>(للأدمن)</strong> تظهر قائمة بالعملاء المشتركين مع الباقات والاستشارات المنفذة، ويظهر مصدر كل عميل — <em>(مشترك)</em></li>
<li>يتحقق الأدمن من الروابط المرسلة من الدكتور للعميل للتأكد من إرسالها</li>
...
| 4 | **Customer** | Tech | Clicks the link and enters the consultation chat. | — |
| 5 | **Doctor** | Medical | Accepts the consultation. | <small>• 🟢 "فيتامينات" badge<br>• Source: "من الصيدلية — [pharmacy name]"<br>• **No lab results** — Phase 1 consultation-only flow<br>• Not yet subscribed to a vitamins package *(to be built)*</small> |
| 6 | **Doctor ↔ Customer** | Medical | Discusses the patient's health concerns, symptoms, and lifestyle. Recommends a vitamins program based on symptom profile only. Sends the vitamins package link in the chat. | — |
| 7 | **Doctor** | Medical | Closes the consultation. | — |
...
| 9 | **System** *(to be built)* | Tech | Payment confirmed. Subscription activated. System sends customer a link to request a new vitamins consultation. | <small>• Patient receives: WhatsApp/SMS — "تم الاشتراك بنجاح! يمكنك طلب استشارة جديدة في أي وقت يناسبك" + consultation request link</small> |
| 10 | **Customer** | Tech | Enters a new vitamins consultation linked to their subscription. | — |
| 11 | **Doctor** | Medical | Accepts the vitamins consultation. Reviews symptom profile. Issues a vitamins prescription. | <small>• 🟢 "فيتامينات" badge<br>• Source: "من الصيدلية — [pharmacy name]"<br>• Subscription plan + active status<br>• Health goals + physical info from signup<br>• **No lab results** — Phase 1 *(to be built)*</small> |
| 12 | **Doctor → Patient** | Medical | Issues a vitamins prescription based on symptoms and health goals. | <small>• Patient receives: prescription PDF via WhatsApp</small> |
| 13 | **Doctor → Pharmacy** *(to be built)* | Medical | Sends the prescription directly to the source pharmacy. | <small>• Pharmacy manager receives: prescription PDF via WhatsApp</small> |
| 14 | **Pharmacy Manager** | Ops | Receives the prescription. Dispenses the vitamins. Ships to the customer. | — |

---

## TYPE 3 🔮 Phase 2: Patient Visits Lab Before Results Are Ready → Vitamins Package Must Include a Lab Test — Not Yet Built

> 🚧 **Phase 2 — Not yet implemented.** The pre-test lab customer visits the lab before their results are ready. In Phase 1, we send them the vitamins link after the consultation closes, but cannot prescribe vitamins without blood results. In Phase 2, the vitamins package for this customer type **includes the lab test** — the patient subscribes, the lab test is ordered and run, results come in, and the doctor prescribes the appropriate vitamins based on actual blood data.

| What changes vs Phase 1 | Details |
|------------------------|---------|
| Package price increases to cover lab test | Lab test cost bundled into subscription pricing |
| Patient subscribes first, lab test ordered from within the vitamins package | Requires integration between the vitamins subscription and the lab ordering flow |
| Lab runs panel and uploads results | Doctor receives vitamins consultation + lab results together |
| Doctor prescribes based on actual blood results | Clinically superior to symptom-only prescription |
...
|------------------------|---------|
| Package price increases to cover lab test | Lab test cost bundled into subscription pricing |
| Doctor orders home lab test from within the consultation | Requires "إرسال للمختبر" feature in doctor app + home collection agreement with Jadar |
| Lab uploads results to Labass org portal | Via "طلبات التحليل" tab (built, awaiting backend) |
| Doctor receives free post-test consultation | Not deducted from any package — same lab referral backend (`POST /consultations/create-from-referral`) |
| Prescription refined based on actual blood results | Higher clinical accuracy, better outcomes |

---

## TYPE 2 🔮 Phase 2: Customer Finds Ad (TikTok/Instagram) → No Blood Results → Must Include Lab Test in Package Before Doctor Can Prescribe — Not Yet Built

> 🔮 **Phase 2 — Not yet implemented**: Vitamins cannot be responsibly prescribed without blood results. An ads customer without existing blood work requires a lab test as part of the package before the doctor can prescribe. This flow is planned for Phase 2 when lab testing is bundled into the subscription pricing.
...
<li>تصل إلى العميل رسالة تؤكد نجاح الاشتراك، وتتضمن كذلك رابطًا لطلب استشارة فيتامينات جديدة — <em>(مشترك)</em></li>
<li>يطلب العميل استشارة فيتامينات جديدة مرتبطة بالباقة عبر الرابط المرسل (لا يدخل نفس الاستشارة الأولى) — <em>(مشترك)</em></li>
<li>يدخل العميل إلى الاستشارة الجديدة، ثم يقوم الدكتور بإصدار وصفة طبية بناءً على الأعراض والأهداف الصحية — <strong>لا يوجد تحليل دم في هذه المرحلة</strong> — <em>(مشترك)</em></li>
<li>تظهر الباقة التي اشترك فيها العميل للطبيب داخل تطبيق الدكتور عند قبول الاستشارة — <em>(مشترك)</em></li>
<li><strong>(🚧 Phase 2 — عند الحاجة لتحاليل مخبرية في المرحلة القادمة)</strong> يرسل الدكتور معلومات المريض لمدير المختبر — هذه الخطوة غير مفعّلة حالياً</li>
<li><strong>(🚧 Phase 2 — تابع)</strong> يُجري المختبر التحليل ويرفع النتائج — هذه الخطوة غير مفعّلة حالياً</li>
...
### GAP 8 — Auto-Send Vitamins Link When Post-Test Lab Consultation Closes

**Problem**: When a post-test lab consultation closes, nothing is sent about vitamins. The doctor must manually remember to send the link.

**What we need**: Every time a **post-test** lab consultation closes, the system automatically sends the patient the vitamins package link via WhatsApp. This is a safety net in case the doctor forgot. Pre-test consultations are Phase 2 (the vitamins package for pre-test customers will include the lab test — not applicable in Phase 1).

- Post-test message: "بناءً على نتائج تحاليلك، قد تستفيد من برنامج فيتامينات مخصّص" + link

**Backend**: When a post-test lab consultation closes, auto-send vitamins link to patient via WhatsApp.
...
## Technical Task Breakdown (by Sprint)

> Ordered to ship the 🔴 lab customer flow end-to-end first. Each task references its GAP.

### Sprint 1 — Lab Customer E2E (🔴 PRIMARY)

| # | Task | GAP | Repo | Effort |
|---|------|-----|------|--------|
| 1.1 | Add `Vitamins` to `ConsultationType` enum | GAP 2 | backend | S |
| 1.2 | Add `source` field + `ConsultationSource` enum to Consultation entity | GAP 3 | backend | S |
| 1.3 | Set `source` automatically at consultation creation (lab → `lab_post_test`, bundle → `org_bundle`) | GAP 3 | backend | M |
| 1.4 | On vitamins subscription activation (payment webhook): auto-create consultation, generate long-lived magic link, send WhatsApp | GAP 1 | backend | L |
| 1.5 | Update success page — replace "سيتواصل معك فريقنا" with direct consultation link + "يمكنك الدخول في أي وقت" | GAP 1 | frontend (patient) | S |
| 1.6 | Store survey data (health goals, height, weight, city) to backend on subscribe confirmation | GAP 7 | frontend (patient) + backend | M |
| 1.7 | Expand doctor feed relations to include `subscription`, `subscription.bundle`, `source`, survey data, and lab results PDF from linked consultations | GAP 3 | backend | M |
| 1.8 | Doctor app — show vitamins badge, source label, subscription status, survey answers, lab results PDF on consultation card/detail | GAP 3 | frontend (doctor) | M |
| 1.9 | Auto-send vitamins WhatsApp link when a post-test lab consultation closes | GAP 8 | backend | S |
| 1.10 | Submit WhatsApp template `vitamins_marketing_post_test` to Meta for approval | GAP 8 | ops | S |
| 1.11 | Add `isVitaminPartner` flag to Organization entity | GAP 5 | backend | S |
| 1.12 | `POST /admin/send-prescription-to-pharmacy` + `GET /admin/partner-pharmacies` endpoints | GAP 5 | backend | M |
| 1.13 | Admin dashboard — subscriber list with plan, source, consultation status, prescription status, pharmacy assignment | GAP 4 + 5 | frontend (admin) | L |
| 1.14 | Lab referral backend: `POST /lab-referrals`, `GET /lab-referrals`, `POST /consultations/create-from-referral` (free post-test) | GAP 10 | backend | L |

### Sprint 2 — Pharmacy Flow + Doctor Tooling (🟡 SECONDARY)

| # | Task | GAP | Repo | Effort |
|---|------|-----|------|--------|
| 2.1 | Skip bundle deduction when `consultationType === "vitamins"` in `createConsultationFromBundle` | GAP 2 | backend | S |
| 2.2 | Org portal — add "فيتامينات" option in DoctorTypeSection with "مجانية ولا تُخصم من باقتك" note | GAP 2 | frontend (patient/orgPortal) | S |
| 2.3 | Set `source: pharmacy_vitamins` when pharmacy sends a vitamins consultation | GAP 3 | backend | S |
| 2.4 | Doctor app — "أرسل الوصفة للصيدلية" button for pharmacy-source consultations | GAP 5 | frontend (doctor) | M |
| 2.5 | Doctor app — "إرسال للمختبر" button: enter lab name + manager phone, sends WhatsApp, logs referral | GAP 10 | frontend (doctor) + backend | M |
| 2.6 | Admin dashboard — doctor compliance column: ✅ vitamins link sent / ❌ not sent (detect via `vitaminLinkSentAt` on Consultation) | GAP 4 | backend + frontend (admin) | M |
| 2.7 | On monthly renewal webhook: auto-create new consultation + send WhatsApp reminder to patient | GAP 6 | backend | M |

### Sprint 3 — Growth & Marketing (🟢)

| # | Task | GAP | Repo | Effort |
|---|------|-----|------|--------|
| 3.1 | `POST /admin/marketing/send-vitamins-campaign` — bulk WhatsApp to all non-subscriber patients, rate-limited, with result logging | GAP 9 | backend | L |
| 3.2 | Admin UI — campaign trigger button with user count confirmation + results summary | GAP 9 | frontend (admin) | M |
| 3.3 | Submit WhatsApp template `vitamins_marketing_campaign` to Meta for approval | GAP 9 | ops | S |
